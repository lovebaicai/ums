<template>
  <div class="app-container">
    <div class="filter-container" align="right" style="margin-bottom: 5px;">
      ...
      <el-button class="filter-item" style="margin-left: 5px;" type="primary" icon="el-icon-refresh" @click="userSyncDialog">
        同步用户
      </el-button>
    </div>
    <el-table
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-form ref="form" :model="tempUserData">
          <el-form-item>
            <el-button size="mini" @click="diglogResetUser = false">取 消</el-button>
    ...
    <el-dialog
      width="30%"
      title="同步用户"
      :visible.sync="diglogSyncUser"
      append-to-body
    >
      <span>确定要开始同步用户吗?</span>
      <span slot="footer" class="dialog-footer">
        <el-form ref="form" :model="tempUserData">
          <el-form-item>
            <el-button size="mini" @click="diglogSyncUser = false">取 消</el-button>
            <el-button type="primary" size="mini" @click="syncSubmit()">确 定</el-button>
          </el-form-item>
        </el-form>
      </span>
    </el-dialog>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      diglogResetUser: false,
      dialogAddUser: false,
      diglogSyncUser: false,
      userId: '',
      roleNameData: [],
      existRoleData: [],
      roleValue: [],
  ...
  methods: {
    ...
    resetDiglog(row) {
      this.diglogResetUser = true
      this.tempUserData = Object.assign({}, row)
    },
    resetSubmit() {
      resetUser({ username: this.tempUserData.username }).then(() => {
        this.$message({
          message: '提交成功',
          type: 'success',
          showClose: true,
